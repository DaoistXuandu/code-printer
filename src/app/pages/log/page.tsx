...
export default function Log() {
    ...
    function iterate() {
        if (!status) {
            return (<div className="text-2xl text-center pt-40">Loading....</div>)
        } else {
            if (!data.length) {
                return (<div className="text-2xl text-center pt-40">No Submission Yet..</div>)
            }
            else {
                return (
    ...
    return (
        <div style={{ backgroundColor: "#3A4A59" }} className="w-full min-h-screen h-fit">
            <NavBar status={true} />
            <div className="flex flex-row w-full h-full pl-16 mt-10 pr-16 h-fit pb-20">
